...
def cadastrar_paciente(request):
    profissionais_de_saude = CustomUser.objects.filter(funcao='profissional_saude') #Filtrando o select com o tipo do usuário
    if request.method == 'POST':
        form = CadastrarPacienteForm(request.POST, profissionais_de_saude=profissionais_de_saude)
        if form.is_valid():
            paciente = form.save()
            messages.success(request, f'Paciente {paciente.nome_completo} cadastrado com senha: {paciente.senha}!')
