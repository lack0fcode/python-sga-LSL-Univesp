...
def gerar_senha_paciente(sender, instance, **kwargs):
    ...
    if not instance.senha and instance.tipo_senha:
        ...
        contador = Paciente.objects.filter(
...
def log_user_login(sender, request, user, **kwargs):
    ...
    print(f"Usuário {user.username} (CPF: {user.cpf}) logou-se em {request.META.get('REMOTE_ADDR')}")
    ...
    RegistroDeAcesso.objects.create(
        ...
        user_agent=request.META.get('HTTP_USER_AGENT'), #Obtém informações do navegador
        view_name=request.resolver_match.view_name if request.resolver_match else 'URL não encontrada',
