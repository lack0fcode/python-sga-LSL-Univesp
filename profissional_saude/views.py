...
def painel_profissional(request):
    ...
    """
    pacientes = Paciente.objects.filter(profissional_saude=request.user, atendido=True).order_by('horario_agendamento')
    profissionais = CustomUser.objects.filter(funcao='profissional_saude').exclude(id=request.user.id)
...
def realizar_acao_profissional(request, paciente_id, acao):
    ...
    if acao == 'chamar':
        ChamadaProfissional.objects.create(paciente=paciente, profissional_saude=profissional_saude, acao='chamada')
        return JsonResponse({'status': 'success', 'mensagem': 'Senha chamada com sucesso.'})
    elif acao == 'reanunciar':
        ChamadaProfissional.objects.create(paciente=paciente, profissional_saude=profissional_saude, acao='reanuncio')
        return JsonResponse({'status': 'success', 'mensagem': 'Senha reanunciada com sucesso.'})
    elif acao == 'confirmar':
        paciente.atendido = False  # Marcar como não atendido para sair da lista
        paciente.save()
        return JsonResponse({'status': 'success', 'mensagem': 'Atendimento confirmado com sucesso.'})
    elif acao == 'encaminhar':
        profissional_encaminhar_id = request.POST.get('profissional_encaminhar_id')
        if profissional_encaminhar_id:
            profissional_encaminhar = get_object_or_404(CustomUser, id=profissional_encaminhar_id)
            paciente.profissional_saude = profissional_encaminhar
            paciente.atendido = True  # Para aparecer na lista do outro profissional
            paciente.save()
            return JsonResponse({'status': 'success', 'mensagem': f'Paciente encaminhado para {profissional_encaminhar.first_name} com sucesso.'})
        else:
            return JsonResponse({'status': 'error', 'mensagem': 'Selecione um profissional para encaminhar.'}, status=400)
    else:
        return JsonResponse({'status': 'error', 'mensagem': 'Ação inválida.'}, status=400)
...
def tv2_view(request):
    ...
    """
    try:
        ultima_chamada = ChamadaProfissional.objects.filter(acao__in=['chamada', 'reanuncio']).latest('data_hora')
        ...
        sala_consulta = ultima_chamada.profissional_saude.sala  # Acessa a sala através do profissional
        ...
        historico_chamadas = ChamadaProfissional.objects.filter(acao__in=['chamada', 'reanuncio']).exclude(id=ultima_chamada.id).order_by('-data_hora')[:5]
    ...
    context = {
        ...
        'ultima_chamada': ultima_chamada if senha_chamada else None,  # Passa a última chamada para o template
    }
...
def tv2_api_view(request):
    ...
    try:
        ...
        data = {'senha': '', 'nome_completo': '', 'sala_consulta': '', 'profissional_nome': '', 'id': ''}
