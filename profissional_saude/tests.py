...
class ProfissionalSaudeTests(TestCase):
    ...
    def test_realizar_acao_chamar_success(self, mock_whatsapp):
        ...
        response = self.client.post(
            reverse('profissional_saude:realizar_acao_profissional',
    ...
    def test_tv2_view_with_calls(self):
        ...
        self.assertEqual(response.context['senha_chamada'], self.paciente2)  # Última chamada
        self.assertEqual(response.context['nome_completo'], self.paciente2.nome_completo)
        self.assertEqual(response.context['sala_consulta'], self.profissional1.sala)
        self.assertEqual(response.context['profissional_nome'], self.profissional1.first_name)
        self.assertEqual(len(response.context['historico_senhas']), 1)  # Uma chamada anterior
    ...
    def test_historico_chamadas_limit(self):
        ...
        chamadas_ordenadas = list(ChamadaProfissional.objects.filter(
            profissional_saude=self.profissional1,
